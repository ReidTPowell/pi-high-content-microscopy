...
def main() -> int:
    ...
    manifest, validation, qc = output / "manifest.jsonl", output / "validation.json", output / "qc.json"
    ...
    stages.append(invoke([sys.executable, str(script_dir / "hca_validate.py"), "--manifest", str(manifest), "--config", str(args.config), "--output", str(validation)]))
    stages.append(invoke([sys.executable, str(script_dir / "hca_well_plan.py"), "--manifest", str(manifest), "--output-dir", str(plan_dir), "--workers", str(args.workers)]))
    stages.append(invoke([sys.executable, str(script_dir / "hca_qc.py"), "--manifest", str(manifest), "--output", str(qc), "--sample-size", str(args.sample_size), "--seed", str(args.seed)]))
    ...
    ready = all(stage["returncode"] == 0 for stage in stages[:2])
    ...
    (output / "preconfiguration.json").write_text(json.dumps({"input": str(args.input.resolve()), "config": str(args.config.resolve()),
        "manifest": str(manifest), "well_plan": str(plan_dir / "plan.json"), "qc": str(qc), "review": str(review),
